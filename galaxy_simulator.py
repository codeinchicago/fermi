...
def detect_prob(disc_vol_scaled):
    ...
    if ratio < 0.002: #set very low ratios to probability of 0
        ...
        detection_prob = -0.004757 * ratio**4 +.6681 * ratio**3 - .3605* ratio * \
        ratio ** 2 + .9215 * ratio + .00826
...
def spirals(b, r, rot_fac, fuz_fac, arm):
    ...
    spiral_stars = []
    fuzz = int(.03 * abs(r)) #randomly shift star locations
    theta_max_degrees = 520
    for i in range(theta_max_degrees): #range(0,600,2) for no black hole
        theta = math.radians(i)
        x = r * math.exp(b * theta) * math.cos(theta + math.pi * rot_fac) + random.randint(-fuzz,fuzz) * fuz_fac
        y = r * math.exp(b * theta) * math.sin(theta + math.pi * rot_fac) + random.randint(-fuzz,fuzz) * fuz_fac
